...
template Permutation() {
  ...
  signal intermediate[65][3];

  component lin = LinearLayer();
  for (var j = 0; j < 3; j++) lin.linIn[j] <== permIn[j];
  for (var j = 0; j < 3; j++) lin.linOut[j] ==> intermediate[0][j];

  component extRounds[8];
  for (var k = 0; k < 8; k++) extRounds[k] = ExternalRound(k);

  component intRounds[56];
  for (var k = 0; k < 56; k++) intRounds[k] = InternalRound(k);

  for (var k = 0; k < 4; k++) {
    for (var j = 0; j < 3; j++) extRounds[k].extIn[j] <== intermediate[k][j];
    for (var j = 0; j < 3; j++) extRounds[k].extOut[j] ==> intermediate[k+1][j];
  }

  for (var k = 0; k < 56; k++) {
    for (var j = 0; j < 3; j++) intRounds[k].roundIn[j] <== intermediate[k+4][j];
    for (var j = 0; j < 3; j++) intRounds[k].roundOut[j] ==> intermediate[k+5][j];
  }

  for (var k = 0; k < 4; k++) {
    for (var j = 0; j < 3; j++) extRounds[k+4].extIn[j] <== intermediate[k+60][j];
    for (var j = 0; j < 3; j++) extRounds[k+4].extOut[j] ==> intermediate[k+61][j];
  }

  for (var j = 0; j < 3; j++) permOut[j] <== intermediate[64][j];
}
